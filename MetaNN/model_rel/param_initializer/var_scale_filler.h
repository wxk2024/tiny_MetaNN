...
#ifndef VAR_SCALE_FILLER_H
#define VAR_SCALE_FILLER_H
#include <random>
#include <stdexcept>
#include <type_traits>
...
namespace MetaNN {
    ...
    public:
        ...
        void Fill(TData &data, size_t fanIn, size_t fanOut) {
            ...
            if constexpr (std::is_same<ScaleMode, VarScaleFillerPolicy::ScaleModeTypeCate::FanIn>::value) {
                ...
                static_assert(DependencyFalse<ScaleMode>); //说明因为 ScaleMode 才出错的
            }
            ...
            if constexpr (std::is_same<DistType, VarScaleFillerPolicy::DistributeTypeCate::Uniform>::value) {
                ...
                double stddev = sqrt(m_factor / fan_factor); // 方差
    ...
    // Xavier Filler, use FanAvg Mode:什么都没做，只是继承了父类，父类有所有相关策略的应对方式
    template <typename TPolicyCont = PolicyContainer<>>
