//
// Created by wxk on 2024/11/17.
//

#ifndef COMPOSE_KERNEL_H
#define COMPOSE_KERNEL_H
#include <MetaNN/policies/change_policy.h>

namespace MetaNN {
    // ------------------四个声明语句SubLayer，InternalConnect，InConnect，OutLayer声明语句-------------
    ...
    namespace NSComposeKernel {
        ...
        // 记录了每个层的名称 Tag,对应的类型 Layer 以及相应的 policy 内容
        ...
        namespace NSTPO {
            ...
            // 遍历 TCheckInters 中的元素，从中找到没有任何前驱层信息的子层，并将其放在 PostTags 中
            template<typename TRemainInters, typename TCheckInters, typename TPostTags, typename... T>
            struct InternalLayerPrune {
                using PostTags = TPostTags;   //任何前驱层信息的子层
                using type = TRemainInters;   // 剩余的层
            };
            ...
            template<typename TOrderedSublayers, typename TUnorderedSublayers,
                typename TCheckInternals /*待检查的内部关系*/>
            struct MainLoop {
                // 循环的退出
            ...
            struct MainLoop<SubLayerContainer<TSO...>,
                ...
                using CurInter = InterConnectContainer<TIC, TI...>; // 内部的这些关系
                ...
                using SeparateByTagFun = SeparateByPostTag<SubLayerContainer<TSN...>,
                    SubLayerContainer<TSO...>,
                    SubLayerContainer<>, PostTags>;  // 将 PostTags 中元素添加到已经排好序的 TSO...，从未排好序的 TSN... 中删除
        ...
        // 唯一的特化版本，规定了两个模板参数，必须是 SubLayerContainer 和 InterConnectContainer
        // 使用这两组信息就可以实现拓扑排序的算法了
        // 1. 预处理： 使用 SubLayerPreprocess_ 元函数找到包含在 SubLayerContainer 中，但不在 InterConnectContainer 中的层
        // 2. 主体逻辑
        // 3. 后处理
        ...
        struct TopologicalOrdering_<SubLayerContainer<TSubLayerElems...>,
            ...
            struct SublayerPreprocess_<SubLayerContainer<TSO...>,
                ...
                // 判断 CurTag 这个层是不是在 内部关系 InterConnectContainer 中
                static constexpr bool inInter = TagExistInLayerComps<CurTag, TInterElems...>::value;

                using NewOrdered = std::conditional_t<inInter,
                    SubLayerContainer<TSO...>,
                    SubLayerContainer<TSO..., TCur> >;   // 如果在 InterConnectContainer 中，则不添加到 Ordered 中，否则添加
            ...
            // 3.后处理: 调用 CascadSublayers 将没有后继，有前驱的层，添加到 Ordered 中
            using type = typename NSTPO::CascadSublayers<OrderedAfterMain, RemainAfterMain>::type;
        };

        /// ========= Sublayer Instantiation ===================================================
        namespace NSSI {
            // 它会在内部 对每个子层 调用 SubPolicyPicker 元函数，基于复合层的 policy 推导出每个子层的 policy
            // 返回的是一个 SublayerPolicyContainer 容器。容器中的每个元素又是 SublayerPolicies 类型的容器
            ...
            namespace NSFeedbackOutSet {
                ...
                struct UpdateByTag<true, TTag, SublayerPolicyContainer<TInstRes...>,
                    ...
                    using type = typename UpdateByTag<true, TTag, NewRes, SublayerPolicyContainer<TInstRemain
                        ...> >::type;
                };
        ...
        struct SublayerInstantiation {
            ...
            // 调用 GetSublayerPolicy 来获得取得每个子层的 policy，只是继承了复合层的 policy ，并没有调整
            ...
            /// 复合层想要梯度，但是子层不设置，那么就无法获得梯度，所以这里要检查一下
            using PlainPolicies = PlainPolicy<TPolicyCont>;
            constexpr static bool IsPlainPolicyFeedbackOut = PolicySelect<FeedbackPolicy,
                PlainPolicies>::IsFeedbackOutput;
            ...
            using FeedbackOutUpdate = typename std::conditional_t<IsPlainPolicyFeedbackOut, // 相当于分支
        ...
        void Init(TInitializer &initializer, TBuffer &loadBuffer, std::ostream *log, TSublayers &sublayers) {
            if constexpr (N != ArraySize<TSublayers>) {
                ...
                using LayerInfo = typename std::tuple_element<N, TSublayerInfo>::type;  // 获取当前层的信息 {tag,Layer}
                using NewInitPolicy = SubPolicyPicker<TInitPolicies, typename LayerInfo::Tag>;
                LayerInit<typename LayerInfo::Layer, TInitializer, TBuffer, NewInitPolicy>(
                    *layer, initializer, loadBuffer);
        ...
        struct SublayerArrayMaker {
            ...
            struct SublayerPtrTuple_<std::tuple<TProcessed...>, std::tuple<TCur, TRemain...> > {
                using type = typename SublayerPtrTuple_<std::tuple<TProcessed..., std::shared_ptr<typename
                        TCur::Layer> >,
                    std::tuple<TRemain...> >::type;
            };
    ...
    struct ComposeTopology {
        ...
        static_assert((NSComposeKernel::SublayerTagInOtherArrays<InterConnects, InputConnects,
                          OutputConnects, SubLayers>::value),
        ...
        /// TopologicalOrdering 只不过是 排好序的 SubLayersContainer
        using TopologicalOrdering = typename NSComposeKernel::TopologicalOrdering_<SubLayers, InterConnects>::type;

        // 处理相关 policy 的调整：比如 A->B ,A 需要更新内部参数，那么 B 的 IsFeedbackOutput 为 true
        template<typename TPolicyCont>
        using Instances = typename NSComposeKernel::SublayerInstantiation<TPolicyCont, TopologicalOrdering,
            InterConnects>::type;
    };
    ...
    /*
     * ComposeKernel 可以看作创建复合层的类模板
     * struct WeightSublayer;
     * struct BiasSublayer;
     * using Topology = ComposeTopology<SubLayer<WeightSublayer, WeightPolicy>,
     *                              SubLayer<BiasSublayer, BiasPolicy>,
    ...
    private:
        using TInstContainer = typename TKernelTopo::template Instances<TPolicyCont>;// 一个 tuple 容器，里面包含的每个子层的实例 每个子层包括 tag 和 Layer 类型
        using SublayerArray = typename NSComposeKernel::SublayerArrayMaker<TInstContainer>::SublayerArray;// 一个 tuple 容器，里面包含的 std::shared_ptr<Layer>
    ...
    private:
        SublayerArray sublayers;  // tuple 当中全是  shared_ptr 子层，初始化使用 SublayerMaker
    };
