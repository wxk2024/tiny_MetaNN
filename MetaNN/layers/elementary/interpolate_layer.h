...
namespace MetaNN {
    //-----InterpolateLayer----
    // 功能：接收3个矩阵或矩阵列表，对他们进行 Interpolate 运算
    // 输入：InterpolateLayerInput
    // 输出：LayerIO
    ...
    public:
        ...
        auto FeedForward(const TIn &p_in) {
            ...
            return LayerIO::Create().template Set<LayerIO>(Interpolate(val1, val2, val_lambda)); // 设置上运算模板
        }
