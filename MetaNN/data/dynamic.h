...
namespace MetaNN {
    // DynamicData 是一种地位特殊的数据结构：用于对不同的数据类型进行封装
    // 基类：提供求值的虚接口，提供大小尺寸函数
    ...
    public:
        ...
        DynamicData(std::shared_ptr<DynamicWrapper<TOriData> > data) {
            m_baseData = std::move(data);//子类来了变父类
        }

        // 基类要求的方法，基类针对 Matrix 进行了特化
        ...
        // 所封装的类型的具体行为，则要等到运行期调用时，才能动态体现出来
        template<typename T>
