...
TEST_CASE("sub layer init", "[model_rel param initializer]") {
    struct Tag1;  //子层的标签
    struct Uniform1;
    struct Uniform2;
    MakeInitializer<float, PInitializerIs<Uniform1>, SubPolicyContainer<Tag1, PBiasInitializerIs<Uniform2> > >().
            SetFiller<Uniform1>(UniformFiller{-1.5, 1.5}).SetFiller<Uniform2>(UniformFiller{0, 4});
}
